...
export class MapComponent implements OnInit, AfterContentInit {
  ...
  @ContentChildren(MapPointComponent) mapPoints: QueryList<MapPointComponent> =
    {} as QueryList<MapPointComponent>;
    ...
    if (this.latitude && this.longitude) {
      if (this.mapHeight && this.mapWidth) {
        ...
        const hw = this.getWindowHeightWidth(
          this.mapDiv.nativeElement.ownerDocument
        );
  ...
  private ensureScript() {
    ...
    const script = <HTMLScriptElement>(
      document.querySelector('script[id="googlemaps"]')
    );
    if (script) {
      if (this.isEnabled) {
        this.renderMap();
  ...
  private renderMap() {
    ...
    if (
      (this.mapPoints && this.mapPoints.length) ||
      (this.dataPoints && this.dataPoints.length)
    ) {
      this.renderMapPoints();
    } else {
    ...
    if (this.map && this.isEnabled) {
      ...
      if (mapPoints) {
        for (const point of mapPoints) {
          let markerText = point.markerText
            ? point.markerText
            : `<h3>${point.firstName} ${point.lastName}</h3>`;
          const mapPointLatlng = this.createLatLong(
            point.latitude,
            point.longitude
          ) as google.maps.LatLng;
